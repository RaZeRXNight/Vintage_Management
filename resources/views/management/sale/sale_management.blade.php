<!DOCTYPE html>
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    @include('reusable.head')

    <body>
    @auth
    @include('reusable.navbar')
    <div class="container">
        <h1>Sale Management</h1>
        <p>Welcome to the Sale Management page!</p>
        <p>Here you can manage your sales effectively.</p>
        
        <a href='/sale_management/create_sale' class="btn btn-primary mb-4">Create Sale</a>
        <a href='/sale_management/create_transaction' class="btn btn-primary mb-4">Create Transaction</a>
    </div>
   <div class="container mx-auto">
       <h1 class="text-2xl font-bold mb-4">Sales Management</h1>
       <table class="min-w-full bg-white border border-gray-200">
           <thead>
               <tr>
                   <th class="py-2 px-4 border-b">Sale ID</th>
                   <th class="py-2 px-4 border-b">User ID</th>
                   <th class="py-2 px-4 border-b">Product ID</th>
                   <th class="py-2 px-4 border-b">Quantity</th>
                   <th class="py-2 px-4 border-b">Total Price</th>
                   <th class="py-2 px-4 border-b">Transaction Date</th>
               </tr>
           </thead>
           <tbody>
               @foreach ($sales as $sale)
                   <tr>
                       <td class="py-2 px-4 border-b">{{ $sale->SaleID }}</td>
                       <td class="py-2 px-4 border-b">{{ $sale->UserID }}</td>
                       <td class="py-2 px-4 border-b">{{ $sale->ProductID }}</td>
                       <td class="py-2 px-4 border-b">{{ $sale->Quantity }}</td>
                       <td class="py-2 px-4 border-b">{{ $sale->TotalPrice }}</td>
                       <td class="py-2 px-4 border-b">{{ $sale->TransactionDate }}</td>
                   </tr>
               @endforeach
           </tbody>
       </table>
   </div>
   <div class="container mx-auto">
       <h1 class="text-2xl font-bold mb-4">Transactions Management</h1>
       <table class="min-w-full bg-white border border-gray-200">
           <thead>
               <tr>
                   <th class="py-2 px-4 border-b">Transaction ID</th>
                   <th class="py-2 px-4 border-b">User ID</th>
                   <th class="py-2 px-4 border-b">Quantity</th>
                   <th class="py-2 px-4 border-b">Total Price</th>
                   <th class="py-2 px-4 border-b">Transaction Date</th>
               </tr>
           </thead>
           <tbody>
               @foreach ($sales as $sale)
                   <tr>
                       <td class="py-2 px-4 border-b">{{ $transactions->where('TransactionID', $sale->TransactionID)->first()->TransactionID }}</td>
                       <td class="py-2 px-4 border-b">{{ $transactions->where('TransactionID', $sale->TransactionID)->first()->UserID }}</td>
                       <td class="py-2 px-4 border-b">{{ $transactions->where('TransactionID', $sale->TransactionID)->first()->Quantity }}</td>
                       <td class="py-2 px-4 border-b">{{ $transactions->where('TransactionID', $sale->TransactionID)->first()->TotalPrice }}</td>
                       <td class="py-2 px-4 border-b">{{ $transactions->where('TransactionID', $sale->TransactionID)->first()->TransactionDate }}</td>
                   </tr>
               @endforeach
           </tbody>
       </table>
   </div>
    @else
    <!-- Be present above all else. - Naval Ravikant -->
    <p>You must be logged in to view this page.</p>
    <p>Please <a href="/">login</a> to access your account.</p>
    @endauth
    
    </body>
</html>

